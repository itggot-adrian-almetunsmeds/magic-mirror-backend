...
x.each do |id|
  ...
  q.each do |s|
    ...
    stops.each do |stop|
      z.execute('INSERT INTO transport (user_id, line, rtTime, time, direction, operator, operatorcode) VALUES (?,?,?,?,?,?,?)', id, stop[0], stop[1], stop[2], stop[3], stop[4], stop[5])
    end
  end
end
...
x.each do |id|
  ...
  w.each do |weather|
    z.execute('INSERT INTO weather (user_id, temp, wind_speed, wind_gust, humidity, thunder, symbol, time) VALUES (?,?,?,?,?,?,?,?)', id, weather[:temp], weather[:wind_speed], weather[:wind_gust], weather[:humidity], weather[:thunder], weather[:symbol], weather[:valid_time])
  end
end
